...
def upload_directory(s3_client, local_path: Path, bucket: str):
    ...
    for file_path in csv_files:
        ...
        try:
            logger.info(f"⬆️  Upload : {s3_key} ({file_size_mb} MB)...")
...
def verify_upload(s3_client, bucket: str):
    ...
    if not objects:
        logger.warning("⚠️  Aucun fichier trouvé dans le bucket !")
        return
...
if __name__ == "__main__":
    ...
    try:
        ...
        if errors == 0:
            ...
            logger.warning(f"⚠️  Upload terminé avec {errors} erreur(s)")
